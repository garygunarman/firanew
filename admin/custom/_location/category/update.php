...
class LOCATION_UPDATE{
   ...
   function insert_category($category_name, $description, $category_active, $category_visibility){
      $sql    = "INSERT INTO `tbl_location_city` (`category_name`, `category_description`, `category_active`, `category_visibility`) 
	             VALUES (?, ?, ?, ?)
			    ";
   ...
   function update_category($category_name, $description, $category_active, $category_visibility, $category_id){
      $sql    = "UPDATE `tbl_location_city` SET `category_name` = ?, 
   ...
   function delete_category($category_id){
      $sql    = "DELETE FROM `tbl_location_city` WHERE `category_id` = ?";
	  $stmt   = $this->conn->prepare($sql);
   ...
   function update_visibility($category_visibility, $category_id){
      $sql   = "UPDATE `tbl_location_city` SET `category_visibility` = ? WHERE `category_id` = ?";
	  $stmt   = $this->conn->prepare($sql);
   ...
   function update_order($category_order, $category_id){
      $sql   = "UPDATE `tbl_location_city` SET `category_order` = ? WHERE `category_id` = ?";
	  $stmt   = $this->conn->prepare($sql);
