...
if($get_order->order_payment_method == 'cc'){
   $get_order->order_payment_method = 'Veritrans';
}else{
				  ...
				  <tbody>
				    <tr>
					  <td width="600" style="padding: 25px">
						...
						<br>
						The total amount of <b>'.$currency.' '.price(2, ($get_order->order_total_amount / $get_order->currency_rate)).'</b> has been paid via <b>'.$get_order->order_payment_method.'</b>
						with the following details:<br>
						<br>
						<b>Paid via:</b> '.$get_order->order_confirm_bank.'<br>
						<b>Amount:</b> '.$currency.' '.price(2, ($get_order->order_total_amount / $get_order->currency_rate)).'<br>
						<br>
						Please check at <a href="http://paypal.com" style="color:#0383ae; font-weight:bold;">Paypal</a> dashboard panel. Once the payment has been reviewed by Paypal, go to your admin panel and mark the order as paid.
					  </td>
					</tr>
					<tr>
					  <td>
