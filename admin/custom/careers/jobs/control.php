...
$full_order       = $_get->count_job($search_query, $sort_by, $query_per_page, $cat);
$total_query      = $full_order['total_query'];
$total_page       = $full_order['total_page'];

// CALL FUNCTION
$listing_order    = $_get->get_job($search_query, $sort_by, $first_record, $query_per_page, $cat);
...
// STORED VALUE
echo "<input type=\"hidden\" name=\"url\" id=\"url\" class=\"hidden\" value=\"http://".$_SERVER['HTTP_HOST'].get_dirname($_SERVER['PHP_SELF'])."/career-view\">\n";
...
if(isset($_POST['btn_index_job'])){
   ...
   if($action == 'delete'){
	  ...
	  foreach($career_id as $career_id){
		 ...
		 if($option == 'yes'){
		    $_update->update_visibility(1, $career_id);
		 }else if($option == 'no'){
		    $_update->update_visibility(0, $career_id);
		 }
