...
$_email_get    = new V_EMAIL_ADM_GET();
$_email_update = new V_EMAIL_ADM_UPDATE();
				  ...
				  <tbody>
				    <tr>
					  <td width="600" style="padding: 25px">
						...
						<a style="color: #0383ae" href="'.BASE_URL.'admin/order-detailing/'.$order_number.'">'.$order_number.'</a> on '.format_date($get_order->order_date).' '.substr($get_order->order_date, -9).'.<br>
						<br>
						The total amount of <b>'.$curr_symbol.price($get_order->currency, $get_order->order_total_amount).'
						</b> will be paid via '.$_global_payment_method[$get_order->order_payment_type].'
					  </td>
					</tr>
					<tr>
					  <td>
						    ...
						    <table width="540" border="0" cellspacing="0" cellpadding="0" align="center" style="font-size: 11px; line-height: 16px; padding-top: 7px">
							  <tbody>
								...
								<tr>
								  ...
								  <td style="border-top: 1px solid #ccc; padding-right: 5px; font-size: 14px; text-align: right"><b>'.$curr_symbol.price($get_order->currency, $get_order->order_total_amount).'</b></td>
								</tr>
							  <tbody>
							</table>
					      </td>
