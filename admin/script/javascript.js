...
function selectRowCheck(row){
	//if (select_flag==1){
	var checkbox_status = $("input[type='checkbox']#check_"+row).is(':checked');
...
/* --- CROSS BROWSER --- */
function crossOnclickSearch(){
   
   var counter   = $("input[id$=_search]").each(function() {
                      //retval.push($(this).attr('id'));
                      $(this).keypress(function(e){
					     if(e.which == 13) {
						    var s     = $(this).attr('id');
							var test  = $(this).attr('id').length;
						    var temp  = s.substring(0, (Number(test) - Number(7)));
							
					        //searchQuery('order_number');
							
							var sort_by        = $("#sort_by").val();
							var query_per_page = $("#query_per_page_input").val();
							var value          = $(this).val();
							...
							if (current_category){
							   location.href=url+"/1/"+current_category+"/"+query_per_page+"/"+sort_by+"/"+temp+"-"+value;
							}else{
							   location.href=url+"/1/"+query_per_page+"/"+sort_by+"/"+temp+"-"+value;
							}
						 }
					  });
                   });
...
function activeNAvbar(pages){
   $('#id-nav-'+pages).addClass('active');
}